...
const acceptedFiles = {'application/vnd.ms-excel': ['.xls'], 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': ['.xlsx']}
    ...
    const onDrop = (acceptedFiles: File[], fileRejections: FileRejection[]) => {
        ...
        fileRejections.forEach((fileRejection) => {
            fileRejection.errors.forEach((error) => {
                setFileData({file: fileRejection.file, error: getErrorMessage(error.code)})
            })
        })
    }

    return (
        <Dropzone onDrop={onDrop} maxFiles={1} accept={acceptedFiles} multiple={false} autoFocus={true}>
                ...
                <section {...getRootProps({className: "dropzone"})}>
                    <div className='w-full flex-col'>
                        ...
                        >
                            ...
                            {error && <p className='text-xs text-red-600' aria-describedby="upload-input">{error}</p>}
                            <input {...getInputProps({
                                id: "upload-input"
                            })}/>
