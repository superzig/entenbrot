        ...
        <Table>
            ...
            <TableBody>
                {routingPlan.map((student, personIndex) => (
                    Object.entries(student.assignments).map(([time, assignment], index) => (
                            <TableRow key={`${personIndex}-${time}`} className={(assignment.isWish == null ? 'border-l-2 border-l-yellow-300' : '')}>
                                <TableCell className='font-medium'>
                                    {(index === 0) ? student.class : ''}
                                </TableCell>
                                <TableCell>{(index === 0) ? student.firstName + " " + student.lastName : '' }</TableCell>
                                ...
                                <TableCell className='text-right'>{(assignment.isWish !== null ? (<Check size={15} className="ms-auto" />) : '')}</TableCell>
                            </TableRow>
                        )
                    )
                ))
