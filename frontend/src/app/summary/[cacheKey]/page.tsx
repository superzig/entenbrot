...
const Page = ({params}: { params: { cacheKey: string } }) => {
        ...
        const downloadDocuments = async () => {
            try {
                const response = await fetch(`http://localhost:8000/api/download/documents/${cacheKey}`, {
                    method: 'GET',
                });
        ...
        return (
            <MaxWidthWrapper className='mb-5 mt-10'>
                <div className='flex h-screen flex-col items-center'>
                    <div className='my-10'>
                        <SuccessScore currentScore={score.reachedPoints} totalScore={score.totalReachablePoints}
                                      maxScore={score.maxReachablePoints}/>
                    </div>
                    <Button onClick={downloadDocuments}>Dokumente herunterladen</Button>
